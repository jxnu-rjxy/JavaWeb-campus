 ...
 * 查找单个用户的所有动态
 */
@WebServlet(name = "GetUserDynamicServlet",urlPatterns = "/getUserDynamicServlet")
public class GetUserDynamicServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        int userId = Integer.parseInt(request.getParameter("userId"));

        DynamicService dynamicService = new DynamicServiceImpl();
        //调用DAO查询该用户发布的动态
        List<Dynamic> dynamicList = dynamicService.getUsers(userId);
        //将动态集合以json形式传给前端
